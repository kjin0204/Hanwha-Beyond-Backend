 ...
 *  2. 중계연산(Intermediate Operation) : .filter(), .map().... - 데이터 가공(반환타입  stream)
 *  3. 최종연산(Terminal Operation) : .forEach() - 결과 처리(반환타입 stream x)
 *
 * 설명.
 *  파이프라인의 특징
...
//        strStream1.forEach(s -> System.out.println(s));
        ...
        /* 설명. Stream으로 관리도니느 요소들을 원하는 갯수만큼 반복할 수도 있다. */
        System.out.println("=========== 배열 일부 구간을 Stream으로 변환 ===========");
